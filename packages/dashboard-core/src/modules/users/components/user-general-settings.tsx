...
type GeneralSettingsForm = z.infer<typeof formSchema>;

export function UserGeneralSettings() {
    ...
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
              <FormLabel>
                {t("settings.general.registrationOpen")}
              </FormLabel>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("settings.general.defaultRole")}</FormLabel>
